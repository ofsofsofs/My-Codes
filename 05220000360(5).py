...
continue_ = "y"  # for to continue the loop
while continue_ == "y":
    ...
    for i in range(1, 6):
        ...
        total_set_won += set_won
        total_set_lost += set_lost
        season_points_won += total_points_won
        season_points_lost += total_points_lost
        season_sets_won += total_set_won
        season_sets_lost += total_set_lost

        # the part that I find maximum difference between teams
        points_difference += total_points_won - total_points_lost
...
# printing part for each match
    ...
    print(f"The average of points won per set is {match_points_won/set_played} and points lost per set is {match_points_lost/set_played}")
...
# resetting for to calculate every match
    ...
    match_points_won = 0
    match_points_lost = 0
    continue_ = input("Do you want to continue?(y/n) ")  # taking input for to continue loop
...
print(f"The number of matches finished in 5 set is {five_set_played} and percentage in all matches is %{five_set_played * 100 / match_played}")
